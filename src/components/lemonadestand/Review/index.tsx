		...
		<Container maxWidth='md' disableGutters={true}>
			...
			<Paper>
				<Typography mx={2} gutterBottom variant='h5'>
					First Name: {props?.contactInformation.firstName}
				</Typography>
				<Typography mx={2} gutterBottom variant='h5'>
					Last Name: {props?.contactInformation.lastName}
				</Typography>

				<Typography mx={2} gutterBottom variant='h5'>
					Email Address: {props?.contactInformation.email}
				</Typography>

				<Typography mx={2} gutterBottom variant='h5'>
					Phone Number: {props?.contactInformation.phone}
				</Typography>
				<TableContainer>
					<Table stickyHeader aria-label='sticky table'>
						<TableHead>
							<TableRow>
								<TableCell sx={{ backgroundColor: theme.palette.primary.main }}>
									Lemonade Type
								</TableCell>
								<TableCell
								...
								<TableCell align='right'>
									{props?.lineItems
										.map((_item: LineItem, index: number) => {
											const quantity = isNaN(props?.lineItems[index]?.quantity)
												? 0
												: props?.lineItems[index]?.quantity;
											const totalAmount =
